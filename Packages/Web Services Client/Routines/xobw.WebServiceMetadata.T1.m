...
#classmethod xobw.WebServiceMetadataTableCode
#classcontext xobw.WebServiceMetadata
#include %callout
#include %occExtent
#include %occInclude
...
#import xobw
 &sql(SELECT availRoot,contextRoot,name,proxyClassName,type INTO :%e() FROM xobw.WebServiceMetadata WHERE ID=:%rowid) s sqlcode=SQLCODE q
...
#sqlcompile SELECT=ODBC
 &sql(SELECT %INTERNAL(ID),availRoot,contextRoot,ien,name,proxyClassName,type INTO :d(1),:d(2),:d(3),:d(4),:d(5),:d(6),:d(7) FROM xobw.WebServiceMetadata WHERE %ID = :%rowid)
...
%QuickFindPKeyByRowID(%rowid) // Get Primary Key fields from row where SQLRowID=%rowid
 n d,s,subs,ul
#sqlcompile SELECT=ODBC
 &sql(SELECT ien INTO :d(4) FROM xobw.WebServiceMetadata WHERE %ID = :%rowid)
...
BuildPurgeIndices(indices="",build)  ; Create/Delete data from index global(s), return Status Code
 q $$$ERROR($$$MethodNotImplemented,"xobw.WebServiceMetadata::%BuildIndices/%PurgeIndices")
